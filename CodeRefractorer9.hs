...
import Prelude 
import Data.Char
--import Language.Haskell.Interpretter

--main = []--map startConvert [1,2,3]--namesOfFunctions
...
main =
	do
		...
		let functionLines = findAndReplaceAllGlobalVariables (convertToListOfLines $ getFunctionCode ("func2") (lexerTest (textString)))(globalVariables)
		...
		return $ removeUnusedVariables(line:lines) -- $ startParsingTheData (localVariables)(drop 1 newAnalyzedLines)--startParsingTheData(localVariables) $ drop 1 newAnalyzedLines
...
startParsingTheData (localVariables)(line:lines)
	| containsBool(line) = startParsingTheData(localVariables)(handleIfStatements(localVariables)(drop 1  $ take (length newLine - 3) $ ls)(lines))
...
handleIfStatements(localvariables)(l:ls)(lines) = deleteBlock(startConvertToBool(l:ls))(lines)
...
findCloseBracket2(line:lines)(n)
	...
	| lineContains (line)(CloseCurly) && n /= 0 = findCloseBracket2(lines)(n-1)
	| lineContains (line)(OpenCurly) = findCloseBracket2(line:lines)(n+1)
