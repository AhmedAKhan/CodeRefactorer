...
main =
	do
		let stepsFile0 = "First It will go to all the functions and condense them into one line\n"
		results <- getAllReturns
		let stepsFile1 = scriptForGettingTheReturns (stepsFile0)
		let stepsFile2 = stepsFile1 ++ "now it will lex the returned data\n"
		let lexedLines = (map lexer) (results)
		let stepsFile3 = stepsFile2 ++ "Now it will replace all the inputParam variables with their associative var(n) value\n"
		let resultAsLexedLines = replaceInputParamOfLines(1)(lexedLines)
		let stepsFile4 = stepsFile3 ++ "converting all the lexed data back into a list\n"		
		let resultAsList = map(convertListToString)(resultAsLexedLines)
		let stepsFile5 = stepsFile4 ++ "combining all the seperate strings into one\n"		
		let finalResult = combineAllStrings resultAsList
		let stepsFile6 = stepsFile5 ++ "Now it is puting the result into a badCode.java file. into a java code template "
		createOutputFile ("return " ++ finalResult)
		writeFile ("steps.txt")(stepsFile6)
