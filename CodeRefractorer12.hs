...
startConvert theCode name=
	do
		let textString = theCode --textString <- badCode
...
refactorMainFunction theCode results =
	do 
		let allLines = convertToListOfLines $ lexerTest (theCode)
		let functionLines = getFunctionCode2 ("badCode")(allLines)
		let globalVariables = getGlobalVariables (allLines) (getGlobalVariablesNames allLines)
		globalVariables
		let newFunctionLines = drop (1+(length globalVariables))(functionLines)
		return (globalVariables)

refractorMainFunction2 =
	do
		results <- main
...
main2 = 
	do
		theCode <- badCode
		--result <- (map (startConvert (theCode)) (["func1", "func2"]))
		--let result =(map (startConvert (theCode)) (["func1", "func2"]))
		result1 <- startConvert theCode "func1"
		--result2 <- startConvert theCode "func2"
		--result3 <- startConvert theCode "func3"
		----result4 <- startConvert theCode "func4"
		--result5 <- startConvert theCode "func5"
		--result6 <- startConvert theCode "func6"
		return $ refactorMainFunction theCode result1
